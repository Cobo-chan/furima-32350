class RegisterInformation
  ...
  validates :postcode, format: { with: /\A\d{3}[-]\d{4}\z/, message: "Postal code Input currectly"}
  validates :prefecture_id, numericality: { other_than: 1, message: "Select" }
  validates :phone_number, numericality: { integer: true, with: /\A[a-zA-Z0-9]+\z/, message: "Phone number Input only number" }
